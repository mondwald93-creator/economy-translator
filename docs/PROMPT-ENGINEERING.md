> ⛔ **무효 (2026-07-31 판정): 스캐폴드 시점(2026년 6월 초)의 일반론 가이드다.**
> 모델 추천(gpt-4, gpt-3.5-turbo), 비용 계산 코드, A/B 테스트 계획은 실제 구현과 다르다.
> 실제 프롬프트 정본 = `src/lib/generateBriefing.ts`(모델은 gpt-4o-mini, 채점 사실 항목만 gpt-4o).
> 품질 채점 정본 = `docs/브리핑_채점기준표.md`(v3.1) + `src/lib/gradeBriefing.ts`.
> 본문은 초기 기록으로 보존한다.
