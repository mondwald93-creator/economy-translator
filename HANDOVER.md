...
> ⛔ **이 문서는 2026-06-16에서 멈춰 있다. 정본이 아니다.**
> 정본은 같은 폴더의 `CLAUDE.md`다. 먼저 그쪽을 읽는다.
>
> 🔴 **특히 "9시 크론 끄기"는 실행하면 안 된다.**
> 2026-07-02 개편으로 역할이 뒤바뀌었다. cron-job.org의 9:00(수집)·9:07(브리핑)이 지금은 **실질 주 생성자**다.
> 끄면 매일 브리핑이 통째로 안 만들어진다. Vercel 크론은 KST 9:20에 도는 보험일 뿐이다.
> (2026-07-29 실측 확인: `vercel.json`의 `/api/cron`은 `20 0` = KST 9:20. 아래 본문이 말하는 "8시"는 이미 없다.)
>
> 아래 내용은 지우지 않고 그대로 둔다. 무엇이 왜 뒤집혔는지 남기기 위해서다.

> 마지막 업데이트: 2026-06-16 (당시 판단: 발행 8시 확정 + 9시 중복이라 끄기 예정 → **이 판단은 7/02에 폐기됨**)
...
| 자동 업데이트 | ⛔ **아래는 옛 정보다. 현재 구성은 `CLAUDE.md` 참조.**<br>~~매일 한국시간 오전 8시 Vercel 통합작업(`/api/cron`, vercel.json `0 23`=UTC23=KST8)~~<br>~~cron-job.org의 9:00·9:07 작업은 8시와 중복이라 끌 예정(6/17)~~<br>🔵 **현재(2026-07-02 개편 후)**: 주 생성 = cron-job.org 9:00 수집 / 9:07 브리핑(유지). 보험 = Vercel `/api/cron` `20 0` = KST 9:20. 뉴스 추가 수집 = KST 13시·17시·22시 |
...
**⛔ [폐기된 계획] 2026-06-17 아침, 9시 중복 발행 끄기 (8시 단독 발행으로 정리)**

> 🔴 **실행 금지.** 2026-07-02 개편으로 9:00·9:07이 실질 주 생성자가 됐다. 끄면 브리핑이 안 나온다.
> 아래는 당시 판단 기록으로만 남긴다. 할 일이 아니다.
