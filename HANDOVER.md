...
1. **경제 달력** ★★ — 금리 발표일 등 주요 경제 일정 페이지 (`/calendar`)
2. **경제용어 사전 확장** — 현재 40개 → 200개로 늘리기
   - 방법: `POST /api/expand-terms` 엔드포인트 새로 만들고 **로컬에서 호출** (Vercel 타임아웃 때문에 로컬 필수)
   - 로컬 호출 명령어: `curl -X POST http://localhost:3001/api/expand-terms`
...
- ✅ 경제용어 사전 — `/dictionary`, 검색 + 카테고리 필터, 40개 초기 데이터, daily_term 자동 저장
...
NAVER_CLIENT_ID=***REMOVED-NAVER-ID***
NAVER_CLIENT_SECRET=***REMOVED-NAVER-SECRET***
```
