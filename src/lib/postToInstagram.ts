 ...
 */
import { pickOpenerFrom } from './postToThreads'

const API = 'https://graph.instagram.com/v21.0'
const MAX_CAPTION = 2200

/**
 * 인스타 첫 줄 — **존댓말.** 2026-08-25 사용자 결정 = 스레드는 반말, 인스타는 존댓말.
 *
 * 그전까지는 스레드의 `pickOpener`를 같이 써서 인스타 캡션 첫 줄만 반말로 나갔다.
 * 카드 그림·헤드라인·소개·유도가 전부 존댓말인데 첫 줄만 어긋나 있었다.
 *
 * 문구는 **스레드 원문의 어미만 옮긴 것이다. Claude가 새 문장을 지어내지 않았다**
 * (2026-08-20 사고: 지어낸 문장 2개 + 근거 없는 "3분"을 넣었다가 지적받음).
 * 어미가 없어 반말이 아닌 것(5번·금·일)은 스레드와 같은 문장 그대로 둔다.
 * ⚠️ 문구를 늘리거나 바꿀 때는 사용자 확인을 받을 것.
 */
const IG_OPENERS: ((label: string) => string)[] = [
  l => `${l} 브리핑 나왔어요! 보러 가실 분🙋‍♀️`,
  l => `오늘도 무사히 발행됐어요... ${l} 브리핑🙋‍♀️`,
  l => `경제 뉴스 봐야지 하고 미루셨던 분 여기요🙋‍♀️\n${l} 브리핑 나왔어요`,
  l => `${l} 경제 뭐 있었냐면요...🙋‍♀️`,
  l => `오늘도 5분컷 가능! ${l} 브리핑🙋‍♀️`,
  l => `경제알못들 모여요~ ${l} 브리핑 왔어요🙋‍♀️`,
  l => `아침에 뉴스 볼 시간 없으셨죠? ${l} 브리핑 요약해왔어요🙋‍♀️`,
]

/** 인스타 요일 전용 첫 줄. 인덱스 = 0=일 … 6=토 (스레드와 같은 순서). */
const IG_WEEKDAY_OPENERS: ((label: string) => string)[] = [
  l => `일요일 느긋하게 ${l} 브리핑 보고 한 주 준비🙋‍♀️`,
  l => `월요일 아침부터 빡세지만... ${l} 브리핑은 보고 가요🙋‍♀️`,
  l => `화요일! 주말 아직 멀었죠... ${l} 브리핑 왔어요🙋‍♀️`,
  l => `수요일이면 절반은 온 거예요 ${l} 브리핑🙋‍♀️`,
  l => `목요일... 하루만 더 버텨요 ${l} 브리핑🙋‍♀️`,
  l => `드디어 금요일! ${l} 브리핑 보고 주말 시작🙋‍♀️`,
  l => `토요일에도 발행됐어요 ${l} 브리핑🙋‍♀️`,
]

/** 그날의 인스타 첫 줄. 고르는 방법은 스레드와 같고 문구만 다르다. */
export function pickIgOpener(date: string): string {
  return pickOpenerFrom(date, IG_WEEKDAY_OPENERS, IG_OPENERS)
}
...
/**
 * 캡션 = 첫 줄(존댓말 계정 목소리) + 헤드라인(존댓말 원문) + 계정 소개·팔로우 유도 + 해시태그.
 * ⚠️ 첫 줄은 `pickIgOpener`(존댓말)다. 스레드의 `pickOpener`(반말)를 쓰지 말 것 —
 *    2026-08-25 사용자 결정으로 두 채널의 말투를 갈랐다.
 *
